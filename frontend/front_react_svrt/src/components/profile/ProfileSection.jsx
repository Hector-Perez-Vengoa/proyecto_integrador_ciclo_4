...
const ProfileSection = ({
  ...
  const getFormatValue = (field, fieldValue) => {
    ...
    if (field.type === "select" || field.type === "multiselect") {
      if (field.name === "departamentoId") {
        ...
        return (
          profile?.cursos?.map((c) => c.nombre).join(", ") || "No especificado"
        );
      }
  ...
  const getFieldOptions = (fieldName) => {
    switch (fieldName) {
      ...
      case "carreraIds":
        ...
        return formData.departamentoId && academicOptions.carreras
          ? academicOptions.carreras
          : [];
      case "cursoIds":
        // Solo mostrar cursos si hay carreras seleccionadas
        return formData.carreraIds &&
          formData.carreraIds.length > 0 &&
  ...
  const getFieldMessage = (fieldName) => {
    switch (fieldName) {
      case "carreraIds":
        return !formData.departamentoId
  ...
  return (
    <div
      className={`bg-white rounded-xl shadow-tecsup border-0 overflow-hidden card-elegant-hover mode-transition-container ${
        editing
          ? "editing ring-1 ring-tecsup-primary/8 glass-effect-elegant"
          : "viewing"
      } edit-mode-transition`}
    >
      {" "}
      {/* Header de la sección */}
      <div className="relative bg-gradient-to-r from-tecsup-primary/5 to-tecsup-secondary/5 px-6 py-4 border-b border-tecsup-gray-100">
        {" "}
            ...
            <div key={field.name} className="space-y-2">
              ...
              <label className="flex items-center gap-2 text-sm font-medium text-tecsup-gray-600 ultra-smooth">
                <div className="w-4 h-4 rounded bg-tecsup-primary/10 flex items-center justify-center ultra-smooth icon-hover-effect">
                  <ProfileIcon
                    iconName={field.icon}
                    className="w-3 h-3 text-tecsup-primary icon-smooth-entrance"
                  />
                </div>
                {field.label}
              ...
              {editing ? (
                ...
                field.type === "select" ? (
                  ...
                  >
                    <option value="">
                      {loadingOptions ? "Cargando..." : field.placeholder}
                    </option>
                ...
                ) : field.type === "multiselect" ? ( // Multiselect mejorado (para carreras y cursos)
                  <MultiSelectField
                    field={field}
                    value={fieldValue}
                  ...
                  <textarea
                    ...
                    className={`w-full px-3 py-2.5 border border-tecsup-gray-200 rounded-lg focus:ring-2 focus:ring-tecsup-primary/20 focus:border-tecsup-primary ultra-smooth resize-none text-tecsup-gray-700 field-focus-glow ${
                      field.readOnly
                        ? "bg-tecsup-gray-50 cursor-not-allowed"
                        : "bg-white"
                    }`}
                  />
                ) : (
                ...
                <div className="px-3 py-2.5 bg-tecsup-gray-50/50 rounded-lg border-0 ultra-smooth">
                  <p
                    className={`text-sm ultra-smooth ${
                      fieldValue
                        ? "text-tecsup-gray-700 font-medium"
                        : "text-tecsup-gray-400 italic"
                    }`}
                  >
                    {formattedValue}
                  </p>
